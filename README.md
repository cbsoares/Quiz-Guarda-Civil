CÓPIA DE SEGURANÇA DO CÓDIGO, SO PRECISA MUDAR O LINK (EXEC).  QUE FICA NA PLANILHA DO GOOGLE EM EXTENSSÕES: APPS SCRIPT.  E FAÇA Á ALTERAÇÃO AQUI NO INDEX, NA LINHA 164

<!-- Index.html — pronto para GitHub Pages, já apontando para sua API /exec -->
<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="utf-8"/>
...
<script>
  /* ===================== CONFIGURE AQUI ===================== */
  // URL pública do seu Web App (Apps Script) - já configurada pra você:
  const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwol4rmJsUOjRV7kzi5KA-nUOHs7X4-NGxBXU_Gx4qFV9b4nTNEMh4max9IKRr9QUe0Tg/exec';

  // Fallbacks (quando não houver template injetado)
  const TEMPLATE_QUESTIONS = null;
  const TEMPLATE_STUDIES = [];
  const TEMPLATE_QUIZ_SIZE = 20;

  // Estado
  let POOL = [];            // perguntas carregadas
  let currentIndex = 0;
  let score = 0;
  let errors = [];
  ...
  function q_topic(q){
    return q.materia || q.Matéria || q.materia || q.topic || '';
  }
  function q_expl(q){
    return q.explicacao || q.explanation || q.Explicacao || q.Explanation || q.explicacao || '';
  }
  function q_answer_letter(q){
    const a = (q.resposta || q.answer || q.Resposta || q.Answer || q.answer || '').toString().trim();
    if (!a) return '';
  ...
  async function renderStudies(){
    ...
    try {
      ...
      if (js && js.ok && js.studies && js.studies.length) {
  ...
  async function showTopicSelection(){
    ...
    try {
      ...
      const js = await res.json();
      if (js && js.ok && js.topics) {
        populateTopicSelect(js.topics || []);
        return;
      } else {
        console.warn('getTopics returned no topics', js);
      }
    } catch (e) {
      console.warn('Erro fetch topics', e);
    }
    if (TEMPLATE_QUESTIONS && TEMPLATE_QUESTIONS.length){
      const t = TEMPLATE_QUESTIONS.map(q=> q.materia || '').filter(Boolean);
      populateTopicSelect(Array.from(new Set(t)));
    }
  }
  ...
  async function loadTopicQuestions(topic){
    ...
    try {
      ...
      if (!data.length && TEMPLATE_QUESTIONS && TEMPLATE_QUESTIONS.length){
        data = TEMPLATE_QUESTIONS.slice(0, TEMPLATE_QUIZ_SIZE);
      }
  ...
  async function renderCards(){
    ...
    container.innerHTML = '';
    try {
      const res = await fetch(`${SCRIPT_URL}?action=getAllQuestions`, {cache:'no-store'});
      const js = await res.json();
      const all = (js && js.ok && js.questions) ? js.questions : (TEMPLATE_QUESTIONS || []);
      makeCards(all);
      return;
    } catch(e){
      console.warn('Erro ao buscar cartões', e);
    }
